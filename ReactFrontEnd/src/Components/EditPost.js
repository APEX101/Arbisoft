...
const EditPost = (props) => {
  // console.log(props.defaultpostid);
  ...
  const handleSubmit = (event) => {
    setResponse("");
    setError("");
    let URL = `http://127.0.0.1:8000/user/posts/${defaultpostid}/`;
    let formdata = new FormData();
    if (postData.body && postImage) {
      formdata.append("body", postData.body);
      formdata.append("picture", postImage.picture[0]);
    } else if (postData.body) {
      formdata.append("body", postData.body);
    } else if (postImage.picture[0]) {
      formdata.append("picture", postImage.picture[0]);
    }
    axiosInstance
      .patch(URL, formdata)
          ...
          <CardContent>
            ...
            <Typography variant="body2" color="textSecondary" component="p">
              ...
              {defaultimage ? (
                <CardMedia
                  className={classes.media}
                  image={`http://127.0.0.1:8000${defaultimage}`}
                  title="Paella dish"
                />
              ) : (
                ""
